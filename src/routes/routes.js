...
router.get('/', async function(req, res) {
    ...
    if (req.session.token) {
        ...
        if(await auth.getUserLevel(profile.id) > 0){
            res.render('../../views/pages/index', { profile: profile, level: req.session.user['level'], page: "home", category: "home" });
        } else {
...
router.get('/add-guru', async function(req, res) {
    ...
    if (req.session.token) {
        ...
        if(await auth.getUserLevel(profile.id) > 1){
            res.render('../../views/pages/add-guru', { profile: profile, level: req.session.user['level'], page: "tambah-guru", category: "admin" });
        } else {
            res.redirect('/unauthorized')        
        }
    } else {
